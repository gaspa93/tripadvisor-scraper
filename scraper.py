...
if __name__ == '__main__':
    ...
    parser.add_argument('--date', type=valid_date, default=datetime.today() - timedelta(days=1), help='Min date of reviews to scrape, format YYYY-MM-DD')
    ...
    if args.q is not None:
        with Tripadvisor(args.date, args.lang, scrape_target=ScrapeType.URL) as scraper:
            scraper.get_urls(args.q)

    # scrape place metadata
    elif args.place:
        with Tripadvisor(args.date, args.lang, scrape_target=ScrapeType.PLACE) as scraper:
    ...
    # scrape place reviews
        ...
        with Tripadvisor(args.date, args.lang) as scraper:
            with open(args.i, 'r') as urls_file:
                for url in urls_file:
